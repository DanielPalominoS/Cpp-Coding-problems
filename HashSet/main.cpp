...
int insert(int value, std::vector<int> &table) {
  ...
  int hash=value%1000;   // use low order bits for addressing
  if(table[hash]==-1)    // empty slot found
  {
    table[hash]=value;     // store value at this index
    return 0;
  }
  else{
    int count=0;
    while(table[hash]!=-1)// && table[hash]!=value)   // walk through chain
    {
      hash++;
      count++;
    }
    table[hash]=value;       // store value at last vacant slot
    return count;
  }
