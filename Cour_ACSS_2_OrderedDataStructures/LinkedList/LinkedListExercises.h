...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  Node *node_to_insert = new Node(newData);
  if (!this->empty())
  {
    // first case if the element should be put in the tail
    if (newData > this->back())
    {
      node_to_insert->prev = this->tail_;
      this->tail_->next = node_to_insert;
      this->tail_ = node_to_insert;
      this->size_++;
      return;
    }
    // first case if the element should be put in the tail
    else if (newData < this->front())
    {
      node_to_insert->next = this->head_;
      this->head_->prev = node_to_insert;
      this->head_ = node_to_insert;
      this->size_++;
      return;
    }
    // fiinally the search is made in the entire list if none of the previous
    // cases arised
    else
    {
      auto nodePtr = this->head_;
      while (nodePtr != this->tail_)
      {
        if (newData > nodePtr->data && newData < nodePtr->next->data)
        {
          nodePtr->next->prev = node_to_insert;
          node_to_insert->next = nodePtr->next;
          nodePtr->next = node_to_insert;
          node_to_insert->prev = nodePtr; // node_to_insert;
          this->size_++;
          return;
        }
  ...
  {
    // If list is empty it should update the head and tail pointers with the
    //  address of the node created recently
    this->head_ = node_to_insert;
    this->tail_ = node_to_insert;
    this->size_++;
    return;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T> &other) const
{
  ...
  if (!left.empty() && !right.empty())
  {
    ...
    int size_merge = left.size_ + right.size_;
    auto leftPtr = left.head_;
    auto rightPtr = right.head_;
    //std::cout << "left size: " << left.size_ << std::endl;
    //std::cout << "Right size: " << right.size_ << std::endl;
    //std::cout << "size_merge: " << size_merge << std::endl<< std::endl;
    while (merged.size_ < size_merge)
    {
      ...
      if (leftPtr != nullptr && rightPtr != nullptr)
      {
        ...
        else if (leftPtr->data > rightPtr->data)
        {
      ...
      //std::cout << "current list: " << merged << std::endl<< std::endl;
    }
  }
