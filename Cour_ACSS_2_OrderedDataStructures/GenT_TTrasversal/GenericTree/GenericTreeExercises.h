...
static void treeFactory(GenericTree<int> &tree)
{
  ...
  auto first_level_left = root->addChild(8);
  root->addChild(15);
  auto second_level_left = first_level_left->addChild(16);
  first_level_left->addChild(23);
  second_level_left->addChild(42);
}
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  //TreeNode* node_tree_branch_ptr=rootNodePtr
  while (!nodesToExplore.empty())
  {
    auto q_front=nodesToExplore.front();
    results.push_back(q_front->data);
    if(q_front->childrenPtrs.size()>0){
      for(auto childPtr: q_front->childrenPtrs){
        nodesToExplore.push(childPtr);
      }
    }
    nodesToExplore.pop();
